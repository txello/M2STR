...
def encode(w_input, words = {}, salt = ''):
    try:
        ...
        if len(salt) == 0: raise lib.Error("Error #2: Вы ввели пустую соль!")
        # salt - секретный ключ(соль) для шифрования.
        # если указана пользовательская соль, то заменяется.
        
        words = conf.word(words)
        if len(words) == 0: raise lib.Error("Error #3: Вы ввели пустой словарь!")
        ...
        if len(w_input) == 0: raise lib.Error("Error #1: Вы ввели пустое сообщение!")
        ...
        # len(salt) - количество символов в сумме соли для шифрования.
            ...
            for k in range(len(salt_str)):
                ...
                l *= 3
                # каждые 3 значения списка записывается соль и сумма соли.
            w_input.insert(j,salt[i])
